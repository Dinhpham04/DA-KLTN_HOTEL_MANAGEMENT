...
## Directory Structure
- `hotel-management-be/`: Target backend. **CRITICAL**: Read `hotel-management-be/AGENTS.md` for strict NestJS conventions before writing code.
- `hotel-management-fe/`: Target frontend. **CRITICAL**: Read `hotel-management-fe/AGENTS.md` for strict React/Biome conventions before writing code.
...
As an AI Agent (Codex, Cursor, Windsurf, etc), **DO NOT** attempt to guess the migration or write code from scratch. You **MUST** use the predefined skills in `.agents/skills/` sequentially for every feature migration.
...
2. **Read the sub-AGENTS.md files:** The backend (`hotel-management-be/AGENTS.md`) and frontend (`hotel-management-fe/AGENTS.md`) have very strict linting, formatting (Biome), and architectural rules. Obey them.
