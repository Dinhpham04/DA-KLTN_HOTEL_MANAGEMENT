...
### 3.16 UI Behavior: Gia hạn và đổi trạng thái trên màn hình Sửa đặt phòng

- `extension_time` là số giờ gia hạn thủ công của đặt phòng; màn edit cho chọn từ 1 đến 11 giờ, còn giá trị rỗng khi lưu được quy về `0`.
- Checkbox `checked_delete` chỉ là state phụ của frontend để bật/tắt khu vực đổi trạng thái; dữ liệu nghiệp vụ thật được lưu ở `delete_status`.
- Khi bật checkbox đổi trạng thái, UI mặc định chọn `delete_status = 1` (`Đăng ký nhầm`); khi tắt checkbox, UI xóa lựa chọn và gửi `delete_status = NULL`.
- Khi `checked_delete = true`, bắt buộc phải chọn một trong ba lý do:

| Giá trị | Nhãn trên màn edit | Ý nghĩa nghiệp vụ |
|---------|--------------------|-------------------|
| 1 | Đăng ký nhầm | Xóa mềm / đăng ký nhầm |
| 2 | Hủy | Khách hủy đặt phòng |
| 3 | No-Show | Khách không đến |

- Nếu người dùng lưu khi đang bật đổi trạng thái, frontend phải hiển thị modal xác nhận vì đây là thao tác thay đổi trạng thái reservation, không phải xóa cứng record.
- Khi cập nhật reservation với `delete_status` khác `NULL`, backend bỏ qua kiểm tra overlap phòng theo hành vi legacy vì booking đang được đưa ra khỏi luồng hoạt động thông thường.
...
| 2026-05-15 | Bổ sung logic gia hạn và đổi trạng thái trên màn hình Sửa đặt phòng (extension_time / delete_status) |
...
## 6. Cleaning Shift (Cleans Shift Management)

### 6.1 Data type mapping

| Gia tri | Tieng Nhat | Tieng Viet | Mo ta |
|---------|------------|------------|-------|
| 1 | ???? | Don phong | Cong viec don phong theo tung room |
| 2 | ????? | Don khu vuc chung | Cong viec ve sinh khu vuc chung |
| 3 | ???�???? | Khoa va an toan | Xac nhan pin/key va an toan |

### 6.2 Cleaning status mapping

| Gia tri | Tieng Nhat | Tieng Viet | Mo ta |
|---------|------------|------------|-------|
| 1 | ??? | Chua bat dau | Moi tao task |
| 2 | ??? | Dang don | Da bat dau thuc hien |
| 3 | ???? | Tam dung | Dang tam dung |
| 4 | ?? | Da xong | Hoan tat don dep |
| 5 | ??? | Da kiem tra | Nhan vien check da xac nhan |
| 6 | ?? | Mo lai | Task duoc mo lai de lam tiep |
| 7 | ?? | Da huy | Task bi huy |

### 6.3 UI behavior rules (migrated)

- Man hinh nhat bao ve sinh trong hotel-management dung ngay hien thi truc tiep lam `cleaningDate`, khong con offset `displayDate - 1 day` nhu legacy.
- Cho phep loc theo co so, room type, va co flag `newReserveFlag`.
- 3 section chinh (Room/Common/Key-Safety) hien thi theo bo cuc goc, khong dung tab.
- Moi detail row ho tro:
  - cap nhat staff assignment (main/sub/check)
  - cap nhat comment
  - transition trang thai qua nut hanh dong
  - note thread CRUD theo tung cleaning detail
- Room cleaning table theo source AIC:
  - Bang chinh chi hien thi/checkbox/reorder; nhan vien, ngay du kien va comment duoc sua trong bang phu khi user tick row.
  - Mau nen xanh duong khi co next reserve sat ngay checkout; xanh la khi reserve co `disableReservation`; cot tra phong mau do khi checkout trong ngay.
  - Neu checkout trong ngay va chua thu du key (`returnKeys < rentalKeys`, chua co `keyReturnDatetime`) thi trang thai hien thi "Tra phong hom nay".
  - Sort noi bo theo `newReserveDate`, ngay tra phong, co so/phong, hoac main staff.
- Common area table theo source AIC:
  - Hien thi co so, nhan vien phu trach, noi dung ve sinh (`areaName`) va trang thai sua chua.
  - Nut "Chi khu vuc chung" submit toan bo row type 2 voi `orderNum` theo thu tu hien tai.
- Type 3 (Key-Safety):
  - hien thi trang thai PIN
  - cho phep check `checkSafetyFlag`
  - luu thoi diem `pinRevokedConfirmedAt` khi nguoi dung xac nhan an toan
- Header note shift co:
  - sao chep ghi chu ngay truoc
  - xoa nhanh
  - luu note vao `cleans.note`
- Co anchor navigation giua cac section lon va nut scroll-to-top.

### 6.4 Backend migration notes (NestJS)

- Da bo sung filter/query cho module `cleaning-shift`:
  - `roomTypeIds[]`
  - `newReserveFlag`
  - `sort`, `direction`
- Da bo sung metadata phong trong response detail:
  - `roomTypeId`
  - `roomTypeName`
- Da bo sung metadata doc-only de FE render dung bang AIC:
  - `facilityNo`, `roomMailboxPassword`
  - `reservePeriodFrom`, `reservePeriodTo`, `reserveNoreserveCountAfter`
  - `reserveDisableReservation`, `reserveRentalKeys`, `reserveReturnKeys`, `reserveKeyReturnDatetime`, `reserveCheckoutReceptionistId`
  - `roomDirtyLevel`, `newReserveDate`
- Cac phan gym cleaning / staff attendance / report pdf trong source chua co model Prisma tuong ung, nen chua bat backend flow day du.

### 6.5 Cleaning Shift Report (`/cleaning-shift`)

- Trang nay migrate tu legacy `/cleaning-shift` (`清掃日報`) va dung cho nhan vien ve sinh cap nhat ket qua thuc hien trong ngay.
- Hotel-management da chot convention moi: FE hien thi `reportDate` va truy van task theo dung `cleaningDate = reportDate` de khop voi workflow n8n tao task cho ngay hien tai.
- Target backend khong co endpoint `clean_confirm`, nen FE dung module NestJS hien co:
  - `GET /cleaning-shifts` de lay task theo facility/date.
  - `PATCH /cleaning-shifts/details/:id/status` de cap nhat trang thai bat dau/ket thuc/xac nhan.
  - `PATCH /cleaning-shifts/details/:id` de cap nhat thoi gian bat dau/ket thuc thu cong.
  - Note thread dung CRUD notes cua cleaning detail.
- Khi khong chon co so, FE goi `GET /cleaning-shifts` cho tung facility dang hoat dong va merge `details` de render nhat bao giong source.
- Theo pham vi hien tai cua hotel-management, report page chi render:
  - `ROOM` / Don phong.
  - `COMMON_AREA` / Ve sinh khu vuc chung.
- Cac khoi legacy ve nhan vien di lam/diem danh, gio nghi, thu hoi khoa/an toan, khu khuan gym va PDF preview chua bat lai tren trang nay vi target API/model tuong ung chua day du va da duoc loai bo khoi UI migration hien tai.

## 7. Dashboard Management Tabs (`/dashboard`)

### 7.1 Scope migrated

- Dashboard target da migrate 2 tab quan ly tu legacy dashboard:
  - `daily-reserve` / Nhan phong.
  - `exit-management` / Tra phong.
- Pham vi hien tai bo qua:
  - khu nhan su / diem danh tren dashboard.
  - xuat PDF va preview PDF.
  - cac dich vu legacy chua co model/API trong target nhu gym.

### 7.2 Date behavior

- `date1` tren URL dung cho dashboard header: thong bao, tong hop phong trong, muc tieu phong trong.
- `date2` tren URL dung cho cac tab quan ly.
- Tab nhan phong lay danh sach reservation co `periodFrom` cung ngay voi `date2`.
- Tab tra phong theo legacy lay ngay tra phong la `date2 - 1 day`, va loc theo `earlyExitDatetime` neu co, fallback ve `periodTo`.
- Query `tab` tren URL giu tab dang mo. Chi chap nhan:
  - `daily-reserve`
  - `exit-management`

### 7.3 UI behavior

- Giao dien tab giu style legacy: tab xanh/trang, khung noi dung vien primary, bang nen trang, header bang mau `#EEEEEE`.
- Moi tab co box dem so phong du kien.
- Reservation row co link sang man hinh sua reservation.
- Row da hoan tat nhan/tra phong duoc to nen xam va disable nut thao tac tuong ung.
- Tab nhan phong hien reservation, bai do xe va bai xe dap co ngay vao trung `date2`.
- Tab tra phong hien reservation, bai do xe va bai xe dap co ngay ra trung `date2 - 1 day`.

### 7.4 TableDailyReserve backend mapping

- Legacy `GET daily-reserve?time=YYYY/MM/DD` da duoc migrate sang NestJS:
  - `GET /daily-reserve`
  - query `time` hoac `date` nhan format `YYYY/MM/DD` hoac `YYYY-MM-DD`.
  - query `dashboardFlag=true` hoac legacy `dashboard_flag=1` chi lay reservation co `chargeStaffId` bang staff hien tai.
- Filter list:
  - `periodFrom` nam trong ngay duoc chon.
  - `dataStatus = 1`, `deletedAt IS NULL`, `deleteStatus IS NULL`.
  - bo qua reservation draft, memo va rakuten flag.
- Response gom du lieu can render bang legacy:
  - client/occupier/contact, facility/room, period, next reserve date.
  - `flagOrderFirst`, `rentalTime`, `result`.
  - direct check-in fields, charge staff, check-in receptionist.
  - service flags: deliverybox, parking, bicycle parking, pet, futon.
  - smart lock credential moi nhat: `maskedPin`, `cardCount`, valid window.
- Do target schema chua co `request_details` va `sale_details`, `result` hien tinh theo du lieu hien co:
  - da nhan phong -> `Da nhan`.
  - client tra sau -> `Tra sau`.
  - da confirm -> `Da xac nhan`.
  - con lai -> `Chua xac nhan`.

### 7.5 TableDailyReserve update rules

- Legacy `PUT daily-reserve/{id}` va `PUT daily-reserves` da duoc migrate:
  - `PUT /daily-reserve/:id`
  - `PUT /daily-reserves`
- `rental_keys` khong con la input tren man hinh nhan phong.
- Thay the bang smart lock:
  - `smartLockPin` tao/cap nhat PIN trong `room_pin_credentials`.
  - `smartLockCardCount` la so luong the phong da ban giao.
  - De tranh them field DB moi trong giai do migration, `smartLockCardCount` duoc map noi bo vao count field hien co cua reservation (`rentalKeys`), nhung FE/API khong con goi la `rental_keys`.
- Khi tao/cap nhat PIN hoac nhap so luong the phong > 0 cho reservation co ngay nhan phong da den va chua check-in, backend tu dong check-in neu phong hop le.
- Neu user tick `checkinFlag = true`, backend bat buoc validate:
  - reservation phai co room va `periodFrom`.
  - ngay nhan phong khong duoc o tuong lai.
  - cleaning detail gan nhat cua room phai o trang thai hoan tat/da kiem tra/da huy neu co.
  - reserve truoc do cua cung phong phai da checkout/key-return hoac hop le theo flag hien co.
- Khi check-in thanh cong, backend:
  - set `checkinFlag = true`.
  - set `reserveStatus = CHECKED_IN` neu status hien tai nho hon CHECKED_IN.
  - set `checkedInAt`, `checkinDate`, `contactedFlag`.
  - set `checkinReceptionistId` bang staff hien tai.
  - mark parking/bicycle reserve lien quan `checkinFlag = true`.
- Bulk update tra ve `errors[]` theo index neu co row loi; cac row khac van duoc xu ly.

### 7.6 API mapping

- Daily reserve dung module moi:
  - `GET /daily-reserve` de lay bang nhan phong.
  - `PUT /daily-reserve/:id` de cap nhat tung row.
  - `PUT /daily-reserves` de bulk update.
- Reservation dung module target hien co cho tab tra phong:
  - `GET /reservations` de lay danh sach theo khoang ngay.
  - `POST /reservations/:id/check-out` de tra phong.
- Parking/bicycle dung module target hien co:
  - `GET /parking-status` de lay danh sach parking reserve.
  - `PATCH /parking-reserves/:id` de cap nhat `checkinFlag` / `checkoutFlag`.
  - `PATCH /bicycle-parking-reserves/:id` de cap nhat `checkinFlag` / `checkoutFlag`.
- PDF report va preview PDF van nam ngoai pham vi migration hien tai.

### 7.7 Dashboard cleaning-shift tab

- Tab `cleaning-shift` trong dashboard migrate tu legacy `TabContentCleaningShift`, nhung chi giu 2 section:
  - `ROOM` / Don phong (`部屋清掃`).
  - `COMMON_AREA` / Ve sinh khu vuc chung (`共用部清掃`).
- Khong migrate trong tab dashboard hien tai:
  - Key collection / safety (`TableKeyCollection`, `KEY_SAFETY`).
  - Gym cleaning.
  - PDF report/preview.
- Tab dashboard tai su dung dung component report da migrate cho route `/cleaning-shift`:
  - `CleaningReportTable`.
  - `CleaningReportDialog` cho nut Sua cua bang Don phong va Ve sinh khu vuc chung, migrate tu legacy `CleaningReport`.
  - `useGetCleaningShiftReport` de gom task theo nhieu co so giong `CleaningShiftReportPage`.
- Backend/API mapping:
  - `GET /cleaning-shifts` voi `cleaningDate` bang ngay dang chon tren dashboard va `facilityId` bat buoc.
  - `GET /cleaning-shifts/details/:id` de lay du lieu modal bao cao ve sinh phong.
  - `PATCH /cleaning-shifts/details/:id` de cap nhat staff, scheduled/start/end/finish time, `cleanStatus`, comment va `reportImg1..4`.
  - `POST /upload/image?subfolder=cleaning-reports/{cleaningDetailId}` de upload anh bao cao; FE luu URL tra ve vao cac field `reportImg1..4`.
- Vi backend target yeu cau `facilityId`, hook report goi `GET /cleaning-shifts` theo tung co so roi merge ket qua tren FE.
- Tab dashboard giu filter giong report page: nguoi phu trach, co so, loai phong.
- Khong migrate cac field legacy ve chia khoa trong modal `CleaningReport` (`use_key_type`, `key_return_contact_type`, `key_return_flag`, `key_return_datetime`) vi nhánh moi da thay bang smart lock va thẻ phòng.

## Changelog (supplement)

| Ngay | Noi dung cap nhat |
|------|-------------------|
| 2026-05-05 | Migrate modal `CleaningReport` cho nut Sua trong bang Don phong va Ve sinh khu vuc chung: GET/PATCH detail, upload 4 anh report, cap nhat status/time/staff/comment/areaName |
| 2026-05-05 | Bo sung tab `cleaning-shift` vao Dashboard Management, chi migrate Don phong va Ve sinh khu vuc chung, tai su dung component `/cleaning-shift` |
| 2026-04-30 | Migrate day du backend/frontend cho TableDailyReserve: them `daily-reserve` API, bulk update, smart lock PIN/card count thay `rental_keys`, bo PDF/gym |
| 2026-04-30 | Bo sung logic UI cho 2 tab Dashboard Management: `daily-reserve` va `exit-management`, bo PDF va nhan su theo pham vi migration |
| 2026-04-30 | Bo sung logic UI cho trang Cleaning Shift Report `/cleaning-shift`, dung endpoint `cleaning-shifts` thay cho legacy `clean_confirm` |
| 2026-04-29 | Bo sung UI behavior chi tiet cho CleanRoomTable/CleaningCommonAreasTable va response metadata can thiet de render theo source AIC |
| 2026-04-29 | Bo sung business logic Cleaning Shift: mapping status/type, UI behavior section-based, filter/sort query, va ghi chu pham vi backend chua co gym/attendance/pdf |
